...
class Chat {
        ...
        for ($i = 0; $i < count($mensajes); $i++) {
            if ($mensajes[$i]['recibe'] == 0) {
               $chats .= '<li class="left clearfix"><span class="chat-img pull-left">
                                <img src="../fotos/'.$sesion->obtenerVariableSesion('nombreUsuario').'/'.$resultado[0]['ImgCuenta'].'"" alt="User Avatar" class="img-circle">
                            </span>
    ...
    public function guardarMensaje($datosMensaje) {        
        ...
        if (!$resultado)            
            $utilidades->mostrarMensaje('El mensaje no se ha podido enviar. \n Intente nuevamente');                            
